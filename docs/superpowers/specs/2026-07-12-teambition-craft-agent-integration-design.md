...
> **实现澄清（2026-07-12 Task 3 前置调研补充，不改变以上已批准规则）**：
> `sfcId`/`tfsId`/`stageId` 是设计阶段依据历史 API 文档链接推断的字段命名，本次调研未能在线完整核实它们在
> 官方最新的 Projects & Kanban Task Board (Beta) 中依然精确有效（该文档 ID 当前无法在线渲染出对应内容）。
> Task 3（阶段 0）必须以**实际探测到的工具返回结构**为唯一事实来源，如字段名与本设计不一致（例如新版本改为
> `statusId` 或新增独立的看板列 ID 字段），以真实响应为准，不要为了迁就设计文档而硬编码字段名转换。
> 建议将“原始远端字段名 → 内部规范化字段”的映射逻辑集中在单一归一化函数中，方便后续字段改名时只改一处。
> 详见 `.superpowers/sdd/task-3-research-memo.md`。
...
> **实现澄清（2026-07-12 Task 3 前置调研补充，不改变以上已批准规则）**：
> 官方最新 User MCP 文档明确说明生成的链接“具有有效期，过期后将自动失效”，这是与“怀疑泄露后手动删除”并列的独立场景，
> 不只是泄露才会失效。集成层应把“Token 过期”和“Token 被撤销/泄露删除”统一归类为同一种可重试的“需要重新认证”
> 错误路径，UI 侧应尽量提前显示预期到期时间、提醒用户续期，而不是等调用失败才发现。
> 另外，User MCP Server 经核实使用 **Streamable HTTP** 传输协议（非 stdio、非纯 SSE），Task 3 构造运行时
> MCP 连接时需要确认现有 MCP client 基础设施支持该协议，如仅验证过 stdio/SSE 路径需要额外验证或扩展。
> 详见 `.superpowers/sdd/task-3-research-memo.md`。
